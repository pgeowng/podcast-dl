...
module.exports  = class Tasks {
  ...
  loadPlaylist(ep, providerPlaylist, newFileName = "playlist.m3u8") {
    const [tsaudioFile, playlistText] = playlist(txt, this.wd.tsaudioName)

    ep.ctx.tsaudioUrl = new URL(tsaudioFile, ep.ctx.playlistUrl)
    this.wd.playlist(text.replace(regexp, newFileName));
  }
